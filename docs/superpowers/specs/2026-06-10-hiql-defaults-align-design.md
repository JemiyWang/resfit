...
1. **对齐口径 = 折中**:
   - A/B 已有结论的按结论:`geometric` value、`clamp_to_goal` high_actor 设默认(已验证有益);
     `use_layer_norm` 保持关(A/B 实测 V(s,g=s) 退步)。
...
| 6 | `--use_layer_norm` | `train_hiql_gc_value.py` | `0` | `0`(不变) | ❌ A/B 实测退步(V(s,g=s) −0.96→−3.20) | — |
...
1. 重训 value(不带 flag = 新默认 geom+hiql+不 LN)→ `outputs_chunk/three_piece_gc_value_aligned.pt`
