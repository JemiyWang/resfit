#!/usr/bin/env bash
# lifttray bcdecay01to001:对齐刚跑的 pouring bcdecay01to001(action_scale 0.05 + bc 0.1→0.01 衰减)。
#   lifttray 本就用 action_scale 0.05;相对现有 lifttray_*_bcdecay(bc 0.1→0)的改动:bc 终值 0.0→0.01。
# stage3-only:复用现有 lifttray 三件套(gc_value/high_actor/act_feat)+ 复用现有 valid offcache
#   (action_scale 0.05、gc_value/way_steps 全同、bc 不进签名 → 命中秒读,不重建 62G;
#   offcache 静态无写入者,只读 memmap 与实验1并发安全)。
# lifttray 特有保持不动:hdf5 路 + no-stage(绝不加 --stage_balanced/--offline_stage_cache)。
# 用法: setsid bash run_lifttray_hiqlv512_sg15_bcdecay01to001.sh <gpu> > lifttray_hiqlv512_sg15_bcdecay01to001.log 2>&1 < /dev/null &
set -u
GPU=${1:-4}
...
OUT=outputs_chunk/lifttray_actfeat_hdf5_bp_bcdecay01to001_hiqlv512_sg15         # 新(bc 0.1→0.01)
ACTION_SCALE=0.05
gate(){ [ -s "$1" ] || { echo "[lt-dec] FATAL 复用产物缺失: $1"; exit 4; }; }

echo "[lt-dec] gate 复用产物(gc_value/high_actor/act_feat + offcache meta)..."
gate "$CACHE"; gate "$GC"; gate "$HA"; gate "$OFFCACHE/buffer_meta.json"
echo "[lt-dec] START lifttray ($TASK) hiqlv512 sg15 bc:0.1→0.01衰减 action_scale=$ACTION_SCALE  GPU=$CUDA_VISIBLE_DEVICES  $(date '+%F %T')"

# === 主训(与现有 lifttray bcdecay 逐字一致,仅 bc 终值 0.0→0.01 / wandb_name / output_dir)===
$RUN -m resfit.rl_finetuning.chunk_residual.train_chunk_residual \
  ...
  --wandb_project dexmg-chunk-residual --wandb_name lifttray_actfeat_hdf5_bp_bcdecay01to001_hiqlv512_sg15 \
  --output_dir "$OUT"
echo "[lt-dec] ALL DONE lifttray hiqlv512 sg15 bc0.1→0.01 $(date '+%F %T')  out=$OUT"
