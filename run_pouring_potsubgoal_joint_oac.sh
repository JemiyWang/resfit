#!/usr/bin/env bash
# pouring potsubgoal_joint + OAC 探索(A/B 的 +OAC 臂)。
# 与 baseline pouring_actfeat_hdf5_bp_bc01_hiqlv512_sg15_as005_potsubgoal_joint 逐字一致,
...
# 用法: setsid bash run_pouring_potsubgoal_joint_oac.sh <gpu> > pouring_potsubgoal_joint_oac.log 2>&1 < /dev/null &
set -u
GPU=${1:-4}
...
OFFCACHE=outputs_chunk/pouring_actfeat_hdf5_bp_hiqlv512_sg15_as005_potsubgoal_joint_offcache   # 只读复用 baseline
[ -s "$OFFCACHE/buffer_meta.json" ] || { echo "[potsubgoal_oac] FATAL offcache 未建完: $OFFCACHE"; exit 4; }
echo "[potsubgoal_oac] START GPU=$CUDA_VISIBLE_DEVICES  $(date '+%F %T')  (offcache 复用 baseline)"
...
$RUN -m resfit.rl_finetuning.chunk_residual.train_chunk_residual \
  ...
  --wandb_name pouring_actfeat_hdf5_bp_bc01_hiqlv512_sg15_as005_potsubgoal_joint_oac \
  --output_dir outputs_chunk/pouring_actfeat_hdf5_bp_bc01_hiqlv512_sg15_as005_potsubgoal_joint_oac
echo "[potsubgoal_oac] DONE $(date '+%F %T')"
