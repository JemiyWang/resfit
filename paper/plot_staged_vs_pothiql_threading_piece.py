"""Staged-reward (full) vs PotHIQL, seed-level eval curves -- Threading + ThreePiece only.

Derived verbatim from plot_staged_vs_pothiql.py (same visual language, same wandb run ids,
same aggregation / frozen-base / open-marker logic); the ONLY change is that PANELS keeps
just the two tasks Threading and ThreePiece, so this renders a standalone 1x2 figure.

The two arms per panel differ only in the reward the residual RL optimizes:
...
DATA CAVEAT -- the pothiql arm did not run to 500k on these two tasks:
  Threading   pothiql: 2 seeds crashed @240-260k (no 500k run with subgoal on).
...
out_pdf = "/mnt/mnt/data/resfit/paper/figure/fig_staged_vs_pothiql_threading_piece.pdf"
out_png = "/tmp/claude-0/-mnt-mnt-data/fdb052e5-c9da-4bb4-a8e2-f6cf805e6d0d/scratchpad/fig_staged_vs_pothiql_threading_piece.png"
