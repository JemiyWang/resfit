#!/usr/bin/env bash
# lifttray 残差重跑:对齐刚跑的 three_piece(hiqlv512_sg15)——value 换 HIQL-aligned
#   (concat rep / hidden=512 / layers=3 / LN / hiql loss+mask / geometric future),
#   subgoal_way_steps 25→15;额外把 demo-BC 系数从 0.1 线性衰减到 0(--bc_coef_final 0.0,500k 步内归零)。
# lifttray 特有、保持不动:hdf5 路 + no-stage(绝不加 --stage_balanced/--offline_stage_cache),
#   task=TwoArmLiftTray / 数据=two_arm_lift_tray.hdf5 / base=run_e0o0sckj_best:v4。
# 所有产物加新后缀 _hiqlv512(离线两件套)/_hiqlv512_sg15(offcache)/_hiqlv512_sg15_bcdecay(主训),
#   绝不覆盖原 lifttray_*_hdf5 产物与 lifttray_actfeat_hdf5_bp_bc01/best.pt。
# 三步串行:gc_value(复用 act_feat 缓存) -> high_actor(命中缓存) -> 主训 500k(no-stage)。
# 用法: setsid bash run_lifttray_hiqlv512_sg15_bcdecay.sh <gpu> > lifttray_hiqlv512_sg15_bcdecay.log 2>&1 < /dev/null &
set -u
GPU=${1:-5}
...
GC=outputs_chunk/lifttray_gc_value_actfeat_hdf5_hiqlv512.pt                      # 新
HA=outputs_chunk/lifttray_high_actor_actfeat_hdf5_hiqlv512.pt                    # 新
OFFCACHE=outputs_chunk/lifttray_actfeat_hdf5_bp_hiqlv512_sg15_offcache          # 新(会重建)
OUT=outputs_chunk/lifttray_actfeat_hdf5_bp_bc01_hiqlv512_sg15_bcdecay           # 新
gate(){ [ -s "$1" ] || { echo "[driver] FATAL 产物缺失: $1 (上一步失败,停)"; exit 4; }; }

echo "[driver] START lifttray ($TASK) hiqlv512 sg15 bcdecay  GPU=$CUDA_VISIBLE_DEVICES  $(date '+%F %T')"
echo "[driver] GC=$GC"
echo "[driver] OUT=$OUT"
...
echo "[driver] 3) 主训 500k(hdf5,base_policy,no-stage,subgoal_way_steps 15,bc 0.1->0)... $(date '+%T')"
$RUN -m resfit.rl_finetuning.chunk_residual.train_chunk_residual \
  ...
  --output_dir "$OUT"
echo "[driver] ALL DONE lifttray hiqlv512 sg15 bcdecay $(date '+%F %T')  out=$OUT"
