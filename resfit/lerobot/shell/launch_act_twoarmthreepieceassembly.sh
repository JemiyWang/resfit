...
# Launch ACT BC training for TwoArmThreePieceAssembly (DexMimicGen, Panda 双臂).
# 数据集: ankile/dexmg-two-arm-three-piece-assembly
# 本脚本照 launch_act_boxcleanup.sh 的"完整版"补齐(原裸版缺 EGL/GPU/conda,会在第一次 eval 崩)。
#   - eval_video_key=agentview:该环境相机集为 agentview + robot0/1_eye_in_hand;原脚本写 frontview
#     是错的(此环境没有 frontview 相机),已改为 agentview
#   - steps=200000:双臂硬任务,原 50000 偏少
#   - 关键值对照 residual_td3.py 的 ResidualTD3ThreePieceAssemblyConfig(dataset / eval_env / 相机)
...
python -m resfit.lerobot.scripts.train_bc_dexmg \
    ...
    --policy act \
    --batch_size 256 \
    --wandb_project dexmg-threepiece-bc \
    --wandb_enable \
