#!/usr/bin/env bash
# act_feat × LeRobot 数据源 可跑性 smoke(真 lift-tray 数据 + 真 ACT base,GPU 4)。
# 目的:验证 LeRobot 数据源全链能跑通(gc_value → high_actor → train_chunk_residual
# offline buffer + 在线 rollout/eval),非训出好模型。
# task: TwoArmLiftTray, state=38, act_feat state_dim=550, action=24, dim_model=512
# cameras: agentview / robot0_eye_in_hand / robot1_eye_in_hand (自动从 act.config.image_features 取)
set -euo pipefail

export PYTHONPATH=/mnt/mnt/data/resfit_lerobot_wt
export CUDA_VISIBLE_DEVICES=4
export MUJOCO_EGL_DEVICE_ID=0          # 单卡掩码下用逻辑号 0(不是物理 4)
...
CACHE=/tmp/lifttray_act_feat_cache_n2.npz
GC=/tmp/lifttray_gc_value_smoke.pt
HA=/tmp/lifttray_high_actor_smoke.pt
N=2                                    # demo 数(冒烟少量)

RUN="conda run -n residual --no-capture-output python -u"

echo "=== [smoke] lift-tray act_feat LeRobot smoke START ==="
echo "ROOT=$ROOT  DS=$DS  BASE=$BASE"
echo "CACHE=$CACHE  GC=$GC  HA=$HA  N=$N"

# --- 0) 先建一份小 act_feat 缓存(num_demos!=None 时主脚本不落盘,故单独建)---
#     chunk_residual 的 act_feat 子目标强制要 --act_feat_cache 已存在,故必须先有缓存。
echo ""
echo "=== [step0] 建 act_feat 缓存(N=$N demos, state_dim 应=550) ==="
...
assert seqs[0].shape[1] == 550, f"state_dim mismatch: got {seqs[0].shape[1]}, expected 550"
print("[step0] PASS: state_dim=550 confirmed")
PY

echo ""
echo "=== [step1] gc_value(act_feat × lerobot lift-tray),命中上一步缓存 ==="
...
echo ""
echo "=== [step2] high_actor,命中缓存 + 吃 gc_value.pt ==="
...
echo ""
echo "=== [step3] 主训:offline buffer(lerobot/no-stage)+ 在线 rollout/eval ==="
...
$RUN -m resfit.rl_finetuning.chunk_residual.train_chunk_residual \
  ...
  --wandb_mode disabled \
  --output_dir /tmp/chunk_residual_smoke_lifttray_out

echo ""
echo "[smoke] lift-tray DONE"
